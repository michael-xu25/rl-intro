"""
Build HTML comparison report: baseline greedy vs checkpoint pass@16.

Reads:
  - baseline_eval.jsonl:                      greedy pass@1 with full response text
  - eval_checkpoint_no_system_prompt.jsonl:    checkpoint pass@16 aggregate stats

Outputs:
  - analysis/baseline_vs_checkpoint.html
...
def build_html(baseline_records, checkpoint_records):
    ...
    for idx in common:
        ...
        b_pass1 = 1.0 if b_correct else 0.0
    ...
    n_checkpoint_majority = sum(1 for r in rows if r["c_rate"] > 0.5)
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .problem-header .left { display: flex; align-items: center; gap: 12px; flex: 1; }
  .problem-header .idx { font-weight: 700; color: #60a5fa; min-width: 60px; }
  ...
  .expand-all { background: #1e293b; border: 1px solid #334155; color: #94a3b8;
...
""")
    ...
    parts.append(f"""
<h1>Baseline vs Checkpoint: 100 GSM8K Problems</h1>
<p class="subtitle">Baseline = greedy (temp=0) pass@1 &nbsp;|&nbsp; Checkpoint = 16 samples (temp=0.7) pass rate &nbsp;|&nbsp; No system prompt</p>
...
<div class="summary-grid">
  ...
  <div class="stat-card">
    <div class="value" style="color:#60a5fa">{avg_cp_rate*100:.1f}%</div>
    <div class="label">Checkpoint Avg Pass Rate</div>
  </div>
  <div class="stat-card">
    <div class="value" style="color:#22c55e">{cat_counts.get('fixed', 0)}</div>
    <div class="label">Fixed (wrong → right)</div>
  </div>
  <div class="stat-card">
    <div class="value" style="color:#ef4444">{cat_counts.get('broken', 0)}</div>
    <div class="label">Broken (right → wrong)</div>
  </div>
  <div class="stat-card">
...
""")
    ...
    parts.append('<button class="expand-all" onclick="toggleAll()">Expand / Collapse All</button>')
    ...
    for rank, r in enumerate(rows, 1):
        ...
        escaped_response = html.escape(r["b_response"])
...
<div class="problem cat-{r['category']}" data-cat="{r['category']}">
  <div class="problem-header" onclick="toggleProblem(this)">
    ...
    <div class="badges">
      <span class="badge badge-entities">{r['n_entities']} entities</span>
      <span class="badge {b_badge}">Baseline: {b_label}</span>
      <span class="badge badge-rate" style="background:{rate_color}30;color:{rate_color}">
        Checkpoint: {r['c_correct']}/{r['c_total']}
      </span>
  ...
  <div class="problem-body">
    ...
    <div class="comparison">
      <div class="panel baseline">
        ...
        <div>Predicted: <b>{html.escape(r['b_pred'])}</b>
        ...
        <div class="response-text">{escaped_response}</div>
        <div class="meta">
      ...
      <div class="panel checkpoint">
        ...
        <div style="margin-top:12px;font-size:0.85em;color:#94a3b8;">
          <i>Individual checkpoint responses not saved in eval file.<br>
          {r['c_correct']}/{r['c_total']} of 16 sampled completions matched the gold answer.</i>
        </div>
      </div>
    </div>
  </div>
